...
% New feature added is the distance 'rd' which is the minimum distance to avoid collision.

function f = collisionAvoidance(N,r,rd,ifPlot)
    ...
    for i=1:N
        ...
        for j=1:N
            if i~=j
                ...
                if r - dis >= 0.001 
                    posXdot = posXdot + (posX(j) - posX(i))*phi(rd,abs(posX(j) - posX(i)));  % New update function for collision avoidance
                    posYdot = posYdot + (posY(j) - posY(i))*phi(rd,abs(posY(j) - posY(i)));  % New update function for collision avoidance
                end
            end
        end
